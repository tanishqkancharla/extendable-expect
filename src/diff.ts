...
export function unifiedDiff(
	...
	function preprocess(line: string) {
		if (!line || line.match(/\\ No newline/)) return
		...
		if ("-+".includes(char)) {
			...
			previousCount++
			counts[char]++
			if (previousCount === diffLimit) return dim(`${char} ...`)
			else if (previousCount > diffLimit) return
		}
		return line
	}
	...
	if (lines.length > diffMaxLines) {
		const firstDiff = lines.findIndex(
			(line) => line[0] === "-" || line[0] === "+"
		)
	...
	let formatted = lines.map((line: string) => {
		...
		if (line[0] === "-") {
			...
			if (isCompact) return green(line)
			return green(`- ${formatLine(line, outputTruncateLength)}`)
		}
		if (line[0] === "+") {
			line = formatLine(line.slice(1), outputTruncateLength)
			if (isCompact) return red(line)
	...
	if (moreLines) formatted.push(dim(`... ${moreLines} more lines`))
